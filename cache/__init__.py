...
import settings
from cache.backends.base import (InvalidCacheBackendError, CacheKeyWarning, BaseCache)
...
__all__ = [
    'get_cache', 'cache', 'close_cache', 'DEFAULT_CACHE_ALIAS'
]
...
if not settings.CACHES:
    ...
    if engine in backend_classes:
        engine = 'cache.backends.%s' % backend_classes[engine]
    else:
...
# here. You need to execute it manually.
def close_cache():
